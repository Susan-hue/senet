...
from results.tasks import generate_broadsheet
...
class OGRExportView(TenantAPIView):
    ...
    department, Dean in faculty, senate/school admins institution-wide."""

    permission_classes = [CanViewResults]

    def get(self, request, pk):
        result = _visible_result_or_404(request.user, pk)
        pdf = build_ogr_pdf(result)
        return _attachment(pdf, export_basename(result, "pdf"), PDF_CONTENT_TYPE)


class BroadsheetExportView(TenantAPIView):
    """Broadsheet .xlsx for a result. Small classes render inline; classes over
    ``BROADSHEET_ASYNC_THRESHOLD`` are generated by a Celery worker and returned
    as a job to poll and download."""
    ...
    def get(self, request, pk):
        ...
        if class_size > settings.BROADSHEET_ASYNC_THRESHOLD:
            job = ExportJob.all_objects.create(
                institution=result.institution,
                result=result,
                kind=ExportJob.Kind.BROADSHEET,
                filename=export_basename(result, "xlsx"),
                requested_by=request.user,
            )
            generate_broadsheet.delay(str(job.id))
            return success_response(
                ExportJobSerializer(job).data,
                "The class is large; the broadsheet is being generated.",
...
class ExportJobDetailView(TenantAPIView):
    """Poll a broadsheet export job; once completed, streams the stored file."""
